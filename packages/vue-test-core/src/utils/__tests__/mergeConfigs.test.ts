...
describe("mergeConfigs", () => {
  describe("basic merging", () => {
    it("should combine simple objects when they have unique keys", () => {
      const base: Data = { keyA: 1, keyB: 2 };
      const override: Data = { keyB: 3, keyC: 4 };
  ...
  describe("nested objects", () => {
    it("should perform recursive merge when nested objects are present", () => {
      ...
      const override: Data = { global: { stubs: { Icon: true } } };
    ...
    it("should create new object when recursively merging nested objects", () => {
      const base: Data = { a: { b: 1 } };
      const override: Data = { a: { c: 2 } };
  ...
  describe("type conflicts", () => {
    it("should replace value when types differ (object with primitive, primitive with object, etc.)", () => {
      const cases = [
        { base: { a: { b: 1 } } as Data, override: { a: false }, expected: false },
    ...
    it("should replace an array with a primitive value", () => {
      const base: Data = { router: ["old"] };
      const override: Data = { router: false };
    ...
    it("should replace a primitive value with an array", () => {
      const base: Data = { router: false };
      const override: Data = { router: ["new"] };
  ...
  describe("edge cases", () => {
    it("should throw when target is not mergeable", () => {
      expect(() => mergeConfigs(null as unknown as Data, { a: 1 })).toThrow();

      expect(() => mergeConfigs(undefined as unknown as Data, { a: 1 })).toThrow();

      expect(() => mergeConfigs(42 as unknown as Data, { a: 1 })).toThrow();
    });

    it("should throw when source is not mergeable", () => {
      expect(() => mergeConfigs({ a: 1 } as Data, null as unknown as Data)).toThrow();

      expect(() => mergeConfigs({ a: 1 } as Data, 42 as unknown as Data)).toThrow();

      expect(() => mergeConfigs({ a: 1 } as Data, "test" as unknown as Data)).toThrow();
    });
    ...
    it("should overwrite with undefined values", () => {
      const base: Data = { a: 1 };
      const override: Data = { a: undefined };
