import { createMountContext } from '../createMountContext'
import { createPipeline } from '../createPipeline'
import { createMountPipeline } from '../createMountPipeline'
import { presets } from '../../__tests__/utils/mockPresets'
...
describe('Mount Pipeline Integration', () => {
    ...
    describe('Pipeline: Presets merging', () => {
        ...
        test('should use options from default preset when plugin config is missing', () => {
            const result = run(defaultPresets)

            expect(result.plugins.i18n).toEqual(
                defaultPresets.default.defaults.i18n
            )
            expect(result.plugins.pinia).toEqual(
        ...
        test('should use options from default preset when plugin config is an empty object', () => {
            const mountOptions = {
                plugins: {
        ...
        test('should fill missing keys from preset when user provides partial config', () => {
            const mountOptions = {
                plugins: {
        ...
        test('should switch to specific preset via extraOptions', () => {
        ...
        test('should throw error when requested preset does not exist', () => {
        ...
        test('should be undefined for a plugin when no default preset is provided and no options are specified', () => {
            const emptyPresets = {}
        ...
        test('should NOT re-enable plugin when it is explicitly disabled by user even if preset has data', () => {
            const mountOptions = {
                plugins: {
        ...
        test('should include all plugins from default manifest when no preset is specified', () => {
            const result = run(defaultPresets)
            ...
            expect(Object.keys(result.plugins)).toHaveLength(
                expectedKeys.length
            )
        })

        test('should include only plugins from the specific manifest when custom preset is selected', () => {
            const extraOptions = { preset: 'custom' }
            const result = run(defaultPresets, {}, {}, extraOptions)
